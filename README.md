#       Notas em computação e comunicação quântica

[![Python programming language](https://img.shields.io/badge/Python-efefef.svg?logo=data:image/svg%2bxml;base64,&style=flat)](#)
[![pennylane: 0.40.0](https://img.shields.io/badge/pennylane-0.40.0-efefef?style=flat)](#)

>   **Autor:** [Carlos Antunis Bonfim da Silva Santos](https://github.com/carlos-antunis-physics/)
>
>   Este projeto é desenvolvido sob a [Licença MIT](./LICENSE).

>   ##  [**Fundamentos de computação quântica**](1.fundamentos/README.md)
>
>   As tecnologias quânticas, atualmente, tem se mostrado como um *framework* promissor para o desenvolvimento de métodos de computação com um novo paradigma completamente disruptivo ao já fundamentado. Neste capítulo, iremos abordar os "*quanta*" de informação da computação quântica, os *`qubits`*, ademais às *`portas lógicas quânticas`* (operações sobre os `qubits`).

<!--
>   ##  [**Algoritmos quânticos**](2.algoritmos-quanticos/README.md)
>
>   Fazendo-se valer das vantagens do paradigma quântico de computação, a exemplo de seu paralelismo natural, alguns algoritmos foram propostos. Neste capítulo, iremos nos aprofundar em tais algoritmos, como os famosos algoritmo quântico de busca proposto pro Groover e o algoritmo de Shor para fatoração de números em fatores primos.
-->

<div align="center">

![under construction image](./under-construction.png)

</div>

##  Referências bibliográficas

[1] *Bouwmeester, Dirk*, e *Anton Zeilinger*. **The physics of quantum information: basic concepts**.

[2] *Nielsen, Michael A.*, e *Isaac L. Chuang*. **Quantum computation and quantum information**.

[3] *Maziero, Jonas*. [**Computação quântica em qiskit - SBF 2024**](https://github.com/jonasmaziero/computacao_quantica_qiskit_sbf_2024).